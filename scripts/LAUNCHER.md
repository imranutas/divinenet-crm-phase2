...
This18September copy requires first-use administrator setup and signed-in local access. See ../START_HERE.md for the current campaign, banner, intake, backup and restore workflow.

Double-click **START-CRM.cmd** on Windows with Node.js 24 and the included backend dependencies. It opens `http://127.0.0.1:3192/#campaigns` after readiness succeeds. It does not install, download or expose anything publicly.
...
The batch file selects an explicitly supplied `DIVINENET_NODE_EXE`, then `runtime/node.exe` if bundled, then Node on PATH. Node must be version 24. It never modifies PATH or PowerShell execution policy. If dependencies are absent, restore `backend/node_modules` from this package. An optional operator-chosen `npm ci --prefix backend` requires network access; the launcher never runs it automatically. The core is portable without optional AI files; the shipped native database dependency must match the target OS/architecture.
