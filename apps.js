...
function renderDashboard() {
  ...
  recent.firstChild.append(link("View all campaigns â†’","#campaigns"));recent.append(campaignTable(state.campaigns.slice(0,5),true));
  ...
  const studio=panel("Creative, with a review step");studio.append(node("p",state.ai.message,"muted"),link("Open creative studio â†’","#studio"));
...
function renderLeads() {
  const note=node("div","Development pipeline: New â†’ Contacted â†’ Qualified. Stage transitions are recorded. Scoring policy and Phase 1 conversion are not approved or connected; no lead is marked Converted.","callout warning");$("view").append(note);
  ...
  function show() {
    ...
    for(const stage of stages){
      ...
      for(const lead of rows) {
        const card=node("article",null,"lead-card");card.append(node("h3",lead.name),node("p",lead.email),node("p",lead.id+" Â· "+campaignName(lead.campaignId)),badge(lead.sourcePlatform),node("p","Consent: "+lead.consentStatus),button("Edit",()=>openLead(lead)),button("History",()=>viewHistory(lead)));
...
function renderStudio(){
  ...
  form.addEventListener("submit",async event=>{
    event.preventDefault();generate.disabled=true;campaign.input.disabled=true;prompt.input.disabled=true;generate.textContent="Generatingâ€¦";error.hidden=true;
