...
function validateLead(lead) {
  ...
  if (!emailPattern.test(String(lead.email).trim())) {
  ...
  if (!allowedPlatforms.includes(lead.sourcePlatform)) {
  ...
  if (
    lead.stage &&
    !allowedStages.includes(lead.stage)
  ) {
    return "Stage must be New, Contacted or Qualified";
  }
...
function generateLeadId() {
  ...
  const numbers = leads
    .map((lead) =>
      Number(String(lead.id).replace("LEAD-", ""))
    )
    .filter((number) => Number.isFinite(number));
...
app.get("/api/campaigns/:id", (request, response) => {
  const campaign =
    campaignRepository.getById(request.params.id);
...
app.post("/api/campaigns", (request, response) => {
  ...
  const newCampaign = {
    ...
    campaignName:
      String(request.body.campaignName).trim(),
    ...
    client:
      request.body.client
        ? String(request.body.client).trim()
        : "",

    brand:
      request.body.brand
        ? String(request.body.brand).trim()
        : "",

    objective:
      request.body.objective
        ? String(request.body.objective).trim()
        : "",

    targetAudience:
      request.body.targetAudience
        ? String(request.body.targetAudience).trim()
        : "",
...
app.delete("/api/campaigns/:id", (request, response) => {
  ...
  campaignRepository.remove(request.params.id);
...
app.get("/api/leads/:id", (request, response) => {
  const lead =
    leadRepository.getById(request.params.id);
...
app.post("/api/leads", (request, response) => {
  ...
  if (!campaign) {
    return response.status(400).json({
      success: false,
      message: "campaignId must reference an existing campaign"
    });
  }
  ...
  const newLead = {
    ...
    phone:
      request.body.phone
        ? String(request.body.phone).trim()
        : "",
...
app.put("/api/leads/:id", (request, response) => {
  ...
  const updatedLead = {
    ...
    // These values cannot be changed through
    // the general lead update endpoint.
    id: current.id,
    stage: current.stage,
    score: current.score,
    scorePolicyVersion:
  ...
  updatedLead.name =
    String(updatedLead.name).trim();

  updatedLead.email =
    String(updatedLead.email)
      .trim()
      .toLowerCase();

  updatedLead.phone =
    updatedLead.phone
      ? String(updatedLead.phone).trim()
      : "";
...
app.patch(
  ...
  (request, response) => {
    ...
    if (!allowedStages.includes(request.body.stage)) {
      return response.status(400).json({
        success: false,
        message:
...
app.delete("/api/leads/:id", (request, response) => {
  const deleted =
    leadRepository.remove(request.params.id);
